...
NPU 指标统一按单卡任务统计。`ASCEND_RT_VISIBLE_DEVICES=5` 运行验证中, torch_npu 可见设备数为 1, 物理 NPU 5 上出现该任务进程, Python 内部设备名为 `npu:0`; 因此训练/推理利用率、HBM 和吞吐均不按多卡汇总。运行时同机其他 NPU 上存在无关进程, 全局 CPU/NPU 占用不作为任务用量依据。表中 HBM 分母沿用对应运行采样记录的 114688MB; 同型号卡在该环境中 `npu-smi` 显示单卡 HBM 为 131072MB, 不改变已记录任务峰值。
...
- Host CPU 参与范围来自代码路径, 不用整机占用推断。训练路径中 `DatasetMapperWithBasis` 在 host 侧读图、resize/flip、处理标注并创建 CPU tensor, 随后模型输入迁移到 NPU; 训练命令 `DATASETS.TEST=()` 时不执行 NMS/SOAP。评测路径中 `ml_nms` 将 boxes/scores/classes `detach().cpu()` 后调用 `torchvision.ops.batched_nms`, 再把索引转回 NPU; `condinst` 后处理包含 `.cpu().numpy()`、RLE encode 和 `pycocotools` bbox 计算; `tools/SOAP.py` 的 SOAP/COCOeval 也在 host CPU 上执行。
- 任务配置的 CPU 并发: 训练和推理命令均设置 `DATALOADER.NUM_WORKERS=0`, 因此没有额外 dataloader worker 进程; 单卡训练/推理为单主 Python 进程。PyTorch/MKL/OpenMP、NumPy/OpenBLAS 线程池仍会在主进程下创建工作线程, 不能把 dataloader worker 数等同于 CPU 实际用核数。
- 实际 host CPU 核心占用: 对 NPU 5 单卡 20 iter 训练的目标 Python 进程按 0.2s 间隔采样 `/proc/<pid>/task/*/stat`, 只统计该 PID 下有 CPU tick 增量的线程及其调度 CPU。该训练段 `DATALOADER.NUM_WORKERS=0`, `IMS_PER_BATCH=2`, 17 个计时 iteration 平均 0.5869s/iter; 采样到最大同时活跃 133 个线程、130 个 CPU core, 全程迁移覆盖 185 个 CPU core。CPU 任务用核以“最大同时活跃 CPU core=130”描述, 185 个 core 是调度迁移覆盖范围, 不是同时用核。
- x86 AVX: 任务进程内 `torch.backends.cpu.get_cpu_capability()` 与 `torch._C._get_cpu_capability()` 均返回 `AVX512`, PyTorch build info 为 `CPU capability usage: AVX512`, 并启用 MKL、MKL-DNN、OpenMP、`PERF_WITH_AVX=1`、`PERF_WITH_AVX2=1`。NumPy runtime 在任务环境中列出 AVX、AVX2、AVX512F/CLX/CNL/ICL 等 SIMD extension, OpenBLAS 架构为 Cooperlake。结合 CPU NMS/后处理代码路径, 可确认该任务的 host CPU tensor/NumPy 路径进入 AVX512-capable runtime; 但宿主 `perf list` 未暴露 retired AVX/SIMD PMU 事件, 因此不能给出 NMS、evaluator 等单个热点的 AVX 指令退休计数。逐热点指令级确认需要可用的 AVX PMU 事件、VTune 或 SDE 类工具。
- Host CPU 与 AICPU 区分: 上述 dataloader、CPU NMS、NumPy/pycocotools 和 SOAP evaluator 都是 x86 host CPU 逻辑, 不是 Ascend 设备侧 AICPU。NPU 5 上使用 `msprof --aicpu=on --task-time=on --ai-core=on` 采集 2 iter 训练, 导出的 `op_summary` 共 10616 条 device 任务: `AI_VECTOR_CORE` 8510、`MIX_AIC` 1052、`MIX_AIV` 674、`AI_CORE` 380; `op_statistic`、`task_time`、`op_summary` 中 AICPU 行数均为 0。训练图内没有观察到设备侧 AICPU fallback。
...
生成了 NPU 5 的 msprof 短训练 trace, 采集项包含 `--aicpu=on --task-time=on --ai-core=on`。该 trace 用于确认训练图设备侧任务类型与 AICPU 是否参与; 完整性能结论仍以 200 iter 训练 metrics、完整评测输出、`npu-smi` 运行期采样、报错栈和代码路径审计为主。

## 5. 阻塞项
| 阻塞点 | 原因 | 是否硬阻塞 | CANN/AscendC 替代方案 | 兜底 |
|---|---|---|---|---|
| ModulatedDeformConv 无原生 NPU 实现 | 原实现依赖 Detectron2/AdelaiDet C++/CUDA 扩展 | 对 CUDA 路径逐算子等价是硬阻塞; 对跑通和指标验证不是硬阻塞 | 基于 AscendC/CANN 实现 DCNv2 / deformable conv, 或接入官方可用 NPU deformable conv | 普通 conv2d 兼容实现, README 指标接近 |
